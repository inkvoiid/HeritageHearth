  ...
  pantries
    ...
    .then(function (pantry) {
      if (pantry) {
        ...
        res.status(404).json(`Error 404: Pantry ${requestedId} not found`);
      }
    })
    .catch((err) => {
...
router.post("/", async function (req, res) {
  ...
  if (pantry) {
    // If pantry is created successfully, send 201 response (created)
    // Add the pantry to the users' pantries
    for (const userID of userAccess) {
      await users
        .updateOne({ username: userID }, { $push: { pantries: id } })
        .exec();
    }
